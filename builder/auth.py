...
class DouyinAuth:
    ...
    def perepare_auth(self, cookieStr: str, web_protect_: str = "", keys_: str = ""):
        ...
        # 如果缺失 s_v_web_id，用 odin_tt 生成一个后备值（抖音需要此参数用于设备指纹校验）
        if "s_v_web_id" not in self.cookie or not self.cookie.get("s_v_web_id"):
            odin = self.cookie.get("odin_tt", "")
            if odin:
                self.cookie["s_v_web_id"] = "verify_" + odin[:32]
            else:
                self.cookie["s_v_web_id"] = "verify_mqdixs8g_nPIMxNIa_KMHK_4OZ6_8Qvt_LXS6aUmx5rvL"
